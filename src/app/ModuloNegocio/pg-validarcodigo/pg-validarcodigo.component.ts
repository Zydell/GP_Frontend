...
@Component({
  selector: 'app-pg-validarcodigo',
  templateUrl: './pg-validarcodigo.component.html',
  styleUrls: ['./pg-validarcodigo.component.css',
    "./../../../assets/vendor/bootstrap-icons/bootstrap-icons.css"
    ]
})
export class PgValidarcodigoComponent {
  seccion: string = '1';
  title = 'GreenPoint';
  code: string = '';
  ...
  ValidarCodigo(form: NgForm) {
      if (form.valid){
        ...
        this.variosServicios.ValCodigo({  codigo: this.code, negocio_id: this.ngc.negocio_id})
        .subscribe(() => {
            this.code = '';
            this.messages2 = [{severity:'success', summary:'Éxito', detail:'Código canjeado exitosamente'}];
          ...
          err => {
            console.error(err);
            this.messages1 = [{severity:'error', summary:'Error', detail:"Código ya canjeado o no pertenece al negocio"}];
