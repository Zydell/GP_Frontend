...
@Component({
  selector: 'app-pg-mapapvnegocio',
  templateUrl: './pg-mapapvnegocio.component.html',
  styleUrls: ['./pg-mapapvnegocio.component.css']
})
export class PgMapapvnegocioComponent implements OnInit {
  negocio: any = {};

  constructor(public  authService: AuthService, private renderer: Renderer2, private http: HttpClient) { }

  async ngOnInit() {
    this.negocio = await this.authService.getNegocio();
    this.loadGoogleMaps();
  }
  ...
  async initMap(): Promise<void> {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(async (position) => {
        ...
        try {
          const response = await this.http.get<any[]>('http://localhost:5000/api/puntos_verdes/negocio/'+this.negocio.negocio_id).toPromise();
          if (!response) {
